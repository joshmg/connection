...
#pragma comment(lib, "../connection/connection.lib")
...
const int DEFAULT_LISTEN_TIMEOUT = 60;                            // wait time in seconds before a listen attempt times out...
...
class p2p {
  private:
    ...
    int LISTEN_TIMEOUT;                         // wait time in seconds before a listen attempt times out...
                                                //   a value of -1 never times out
  ...
  public:
    ...
    void set_link_timeout(int);                           // overwrites the default timeout time used by link()
